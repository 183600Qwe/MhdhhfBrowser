...
function 主页项目项目列表(标题,链接)
  return {text=标题,图标=标题}
end
function 删除(d,v,是否多选,是否为文件夹,d2,是否为主页项目)
  if 是否为文件夹 then
    ...
    if 是否为主页项目 then
      ...
      table.remove(list3.Adapter.Data,d)
      list3.Adapter.notifyDataSetChanged()
     else
      刷新书签()
      内容2=""
      内容=""
...
function 书签长按菜单(标题,链接,v,是否为主页项目,d,text,是否为文件夹,d2,popadp_text)
  ...
  menu.add("书签信息").onMenuItemClick=function(a)
    local 文本内容="标题:"..标题.."\n链接:"..链接.."\n编号:"..d
    if name2=="历史记录保存" and 时间表[d] then
      文本内容=文本内容.."\n时间:"..时间表[d]
    end
    对话框({
      标题="信息",
  ...
  menu.add("删除书签").onMenuItemClick=function(a)
    删除(d,v,false,是否为文件夹,d2,是否为主页项目)
  end
  pop.show()--显示
end
function 刷新书签(name)
  ...
  if name=="历史记录保存" then
    ...
    for t,c in 内容:gmatch("【项目】(.-)【项目】") do
      ...
      table.insert(title2,t:match("【标题】(.-)【标题】"))
      table.insert(bookId2,t:match("【链接】(.-)【链接】"))
      if t:match("【图片编号】(.-)【图片编号】") then
        图片编号表[a]=t:match("【图片编号】(.-)【图片编号】")
        图片编号表是否有内容=true
      end
    end
  end
...
function showDataDialog(name,title,是否悬浮窗)
  ...
  task(1,function()
    function 设置点击事件()
      listOnItemClick=function(parent,v,pos,id)
        if not 状态5 then
          if name=="历史记录保存" then
            if id<=table.size(历史记录顶部菜单) then
              a=历史记录顶部菜单[id].Text
              运行代码含读取文件("mods/lishidingbucaidan")
             else
              离开布局页面()
              书签和历史记录列表按钮点击事件(id-table.size(历史记录顶部菜单))
            end
           elseif id<=table.size(文件夹列表) then
    ...
    --print(tostring(viewh/(AWidth/70)).."%w")
    item={
      FrameLayout,
      layout_width="fill",
      id="列表按钮外",
    ...
    if (title=="书签") or (title=="历史记录") or 主页链接=="收藏" or 主页链接=="收藏带搜索栏" then
      item书签=item
      if #title2>0 then
        文件夹内容弹窗是否正在显示=false
                ...
                for d in pairs(data) do
                  ...
                  if data[d][3] then
                    aImagea={
                      ...
                      onClick=function()
                        if not 状态5 then
                          查看图片(adpd2[d].Imagea.src)
                        end
                      end,
                      onLongClick=function(v)
                        图片长按事件(v,adpd2[d].Imagea.src,d,data[d][3])
                      end,
                    }
                    if data[d][3]==0 then
                      aImagea.src=picture2..data[d][1]..".png"
                     else
                      aImagea.src=picture2..data[d][1].."("..data[d][3]..").png"
                    end
                    a.Imagea=aImagea
                   else
                    a.Imagea={
                  ...
                  table.insert(adpd2,a)
                end
                return adpd2
              end
              adpd2={}
              local adpd2=adapterData(文件夹内容)
              local items=LuaAdapter(this,adpd2,item)
        ...
        if 图片编号表是否有内容 then
          ...
          function 图片长按事件(v,src,d,图片编号)
            local pop=PopupMenu(activity,v)
            menu=pop.Menu
            menu.add("分享图片").onMenuItemClick=function(a)
              Sharing(src)
            end
            menu.add("复制路径").onMenuItemClick=function(a)
              --先导入包
              import "android.content.*"
              activity.getSystemService(Context.CLIPBOARD_SERVICE).setText(src)
              print("已复制")
            end
            menu.add("图片信息").onMenuItemClick=function(a)
              对话框({
                标题="图片信息",
                点击事件=function()end,
                文本内容="路径："..src.."\n体积："..GetFileSize(src).."\n此标题里的编号："..图片编号.."\n项目编号："..d,
        ...
        function adapterData(data,文件夹列表,icdChecked,是否为反选)
          ...
          if name~="历史记录保存" then
            ...
            历史记录顶部菜单={
              {Text="按照日期查找",Image="png/mmm.png"},
              --{Text="按照域名查找",Image="png/mmm.png"},
              {Text="搜索",Image="png/mmm.png"},
              {Text="关闭的标签页",Image="png/qwetyi.png"},
            }
            for d,b in pairs(历史记录顶部菜单) do
              table.insert(adpd,{
                text={
                  Text=b.Text
                },
          ...
          for d in pairs(data) do
            ...
            if 图片编号表[d] then
              aImagea={
                Visibility=View.VISIBLE,
                onClick=function()
                  查看图片(adpd[d+1].Imagea.src)
                end,
                onLongClick=function(v)
                  图片长按事件(v,adpd[d+1].Imagea.src,d,图片编号表[d])
                end,
              }
              if 图片编号表[d]=="0" then
                aImagea.src=picture2..data[d]..".png"
               else
                aImagea.src=picture2..data[d].."("..图片编号表[d]..").png"
              end
              a.Imagea=aImagea
             else
              a.Imagea={
  ...
  function 刷新主页项目(title)
    ...
    if (title=="书签")==false and (title=="历史记录")==false and 主页链接=="仅搜索栏" then--判断是否主页
      ...
      if 主页项目=="" or 主页项目==nil then
        ...
        --就相当于  for  1,3   do
        table.insert(主页项目datas,主页项目项目列表("书签"))
        table.insert(主页项目datas,主页项目项目列表("历史"))
        for t,c in 主页项目:gmatch("【项目】(.-)【项目】") do
          --给空的datas添加所有的数据
          --格式为  table.insert(空列表名称,{id=数据列表[nj]})
          table.insert(主页项目datas,主页项目项目列表(t:match("【标题】(.-)【标题】"),t:match("【链接】(.-)【链接】")))
        ...
        --将小项目匹配给大list
        list33333.Adapter=yuxuan_adpqy
        --菜单点击事件
        list33333.setOnItemClickListener(AdapterView.OnItemClickListener{
          onItemClick=function(parent, v, pos,id)
            if id<=2 then
              if id==1 then
                打开书签()
        ...
        list33333.onItemLongClick=function(l,v,p,i)
          if not 状态5 then
            local 标题文本=v.Tag.text.Text
            --书签长按菜单(标题文本,主页项目5[标题文本],v,true)
            书签长按菜单(标题文本,主页项目5[标题文本],v,true,i,v.Tag.标题)
          end
        end
      end
    end
